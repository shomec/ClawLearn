...
The entire app natively bridges a custom FastAPI backend serving OpenClaw agents to OpenWebUI for a fluid user experience.

## Tech Stack
- LLM: Ollama (`glm-4.7-flash`)
- Agents: Genuine OpenClaw Daemon (`openclaw daemon`) running locally via Node
- Backend: Node.js (Express) routing custom logic to OpenClaw via child processes
...
2. **Onboard the Genuine OpenClaw Daemon**
...
3. Behind the scenes, the Express.js server invokes the genuine **OpenClaw Agent Daemon** bundled inside the backend container via native Node `child_process.exec` hooks to execute and orchestrate your request natively on the standard OpenClaw interface!